...
本次更新：[v2.1.17 更新说明](docs/releases/v2.1.17.md)。
...
| 产品 | 版本 | 支持环境 | 下载 |
|---|---:|---|---|
| PS2UI Photoshop 导出器 | 2.1.17 | Photoshop 2023+ | [Release](https://github.com/Homer79980/PS2UI/releases/latest) |
...
1. 下载 `PS2UI-Photoshop-2.1.17.ccx`。
...
2. 下载并双击 `PS2UI-Photoshop-2.1.17.ccx`。
3. 在 Creative Cloud 的安装确认窗口中完成安装。
4. 完全退出并重新启动 Photoshop。
5. 从 `插件 -> PS2UI` 打开面板，左上角应显示 `v2.1.17`。
...
```powershell
Get-FileHash .\PS2UI-Photoshop-2.1.17.ccx -Algorithm SHA256
```

公开 Release 提供 CCX 安装包和 `PS2UI-2.1.17-SHA256.txt`，请将本地计算的 SHA-256 与校验文件比对。
